...
const Header = () => {
  ...
  const [loggedIn, setLoggedIn] = useState(!!(typeof window !== 'undefined' ? sessionStorage.getItem('token') : null));
  ...
  {loggedIn ? (
    ...
    <Link to={'/login'}>
      ...
      <ul className={status? 'md:flex':'md:flex justify-center hidden'}>
        <Link to={'/review-home'}><li className='mx-4 mt-3 md:mt-0'>Home</li></Link>
        <Link to={'/review-collections'}><li className='mx-4 mt-3 md:mt-0'>Collections</li></Link>
        {/* <Link to={'/view'}><li className='mx-4 mt-3 md:mt-0'>View</li></Link> */}
        <Link to={'/review-panel'}><li className='mx-4 mt-3 md:mt-0'>Review Panel</li></Link>
      </ul>
    </nav>
